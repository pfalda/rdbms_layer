...
public class DBMSWorker extends UntypedActor {
    ...
    private Vector<ActorRef>    messages_senders = new Vector<ActorRef>();
    ...
    public void closeStatement()
    {
        ...
        {
            System.err.println("CAUGHT EXCEPTION WHILE CLOSING STATMENT: "+ex);
        }
    }
    ...
    {
        DbmsLayer.getConnectionsBroker().tell(new GetDbmsConnectionRequest(),getSelf());
    }
                        ...
                        case ASYNC_READ_WRITE:
                            switch(exec_msg.request_opts.behaviour)
                            {
                                case RESOURCE_GETTER:
                                    ...
                                    DbmsLayer.getAsyncGetTasksBroker().forward(exec_msg, getContext());
                                    return;
                                case RESOURCE_RELEASER:
                                    /* SOMEONE ELSE WILL SEND THE REPLY */
                                    DbmsLayer.getAsyncReleaseTasksBroker().forward(exec_msg,getContext());
                                    return;
                                case RESOURCE_WHATEVER:
                                    /* SOMEONE ELSE WILL SEND THE REPLY */
                                    DbmsLayer.getAsyncWhateverTasksBroker().forward(exec_msg,getContext());
                                    return;
                                default:
    ...
    public void postStop()
    {
        closeStatement();
        if(dbms_connection != null)
            DbmsLayer.getCloseConnectionsBroker().tell(new CloseDbmsConnectionRequest(dbms_connection), ActorRef.noSender());
